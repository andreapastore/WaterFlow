...
public class RestModificaTempiTimer {
	...
	public RispostaLoggedIn modificaTimerQrCode(@PathVariable("numero") int numero, HttpServletRequest request, HttpServletResponse response)
	{
	...
	public RispostaLoggedIn modificaTimerFlussoAcqua(@PathVariable("numero") int numero, HttpServletResponse response, HttpServletRequest request)
	{
		Socio s = (Socio) request.getSession().getAttribute(request.getSession().getId().toString());
		if (s != null)
		{
